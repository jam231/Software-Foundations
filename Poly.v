...
Theorem snoc_as_app : forall (X : Type) (x : X) (l : list X),
  snoc l x =  l ++ [x].
Proof.
  intros. induction l.
  - simpl. reflexivity.
...
Theorem map_app_dist : forall (X Y : Type) (f : X -> Y) (l1 l2 : list X),
...
Proof.
  ...
  induction l.
  - simpl. reflexivity.
  - simpl. rewrite <- IHl. repeat (rewrite snoc_as_app).  
    rewrite map_app_dist. reflexivity.
Qed.
(** [] *)
...
Proof.
  unfold succ. unfold zero. unfold one. reflexivity.
Qed.

Example succ_2 : succ one = two.
Proof.
  unfold succ. unfold one. unfold two. reflexivity.
Qed.

Example succ_3 : succ two = three.
Proof.
  unfold succ. unfold two. unfold three. reflexivity.
Qed.
...
Proof.
  unfold plus. unfold one. unfold zero. reflexivity.
Qed.

Example plus_2 : plus two three = plus three two.
Proof.
  unfold plus. unfold two. unfold three. reflexivity.
Qed.

Example plus_3 :
  plus (plus two two) three = plus one (plus three three).
Proof.
  unfold plus. unfold two. unfold one. unfold three. reflexivity.
Qed.
