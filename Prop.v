...
Proof.
 intros. induction H.
 - simpl.  apply H0.
 - simpl. apply g_plus3 with (n:= n+ m). apply IHgorgeous.
 - simpl. apply g_plus5 with (n:= n+ m). apply IHgorgeous.
Qed.
(** [] *)
...
Theorem ev_ev__ev : forall n m,
  ev (n+m) -> ev n -> ev m.
Proof.
  intros. induction H0. simpl in H.
  - apply H.
...
Inductive pal {X: Type}: list X -> Prop:=
 ...
 | pal_list : forall (x : X) (list : list X) , pal list -> pal (snoc (x :: list) x).

Theorem pal_app_rev : 
  forall (X : Type) (list : list X), pal (list ++ rev list).
Proof.
  intros. induction list. 
  - simpl. constructor.
  - simpl. rewrite <- snoc_with_append. constructor. apply IHlist.
Qed.                     
...
Proof.
  ...
  - reflexivity.
  - reflexivity.
  - rewrite rev_snoc. simpl. rewrite IHpal. rewrite rev_involutive. 
...
Theorem R_is_subset_of_plus :
  forall m n o, R n m o -> m + n = o.
Proof.
  intros. induction H.
  - simpl. reflexivity.
  - rewrite plus_comm. simpl. rewrite plus_comm. rewrite IHR. reflexivity.
  - simpl. rewrite IHR. reflexivity.
  - inversion IHR. rewrite plus_comm in H1. simpl in H1. rewrite plus_comm in H1. 
    inversion H1. reflexivity.
  - rewrite plus_comm. apply IHR.
Qed.

Theorem plus_is_subset_of_R :
  forall m n o, m + n = o -> R m n o.
Proof.
  intros m. induction m. 
  - induction n.
    + intros. simpl in H. rewrite <- H. constructor.
    + intros. simpl in H.  rewrite <- H. constructor. apply IHn. simpl. reflexivity.
  - induction n.
    + intros. rewrite plus_comm in H. simpl in H. rewrite <- H. constructor. apply IHm. rewrite plus_comm. simpl. reflexivity.
    + intros. simpl in H. rewrite <- H. constructor. apply IHm. reflexivity.
Qed.
