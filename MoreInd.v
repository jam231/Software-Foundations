...
Proof.
  apply nat_ind. 
  - (* O *) reflexivity.
  - (* S *) simpl. intros n IHn. rewrite -> IHn. 
    reflexivity.  Qed.
...
(** This proof is basically the same as the earlier one, but a
    ...
    in the original theorem.  This is we have labeled the second
    subgoal just [S] -- if we tried to write it out in the more
    explicit form that we've been using for most proofs, we'd have to
    write [n = S n], which doesn't make a lot of sense!  All of these
    conveniences make [induction] nicer to use in practice than
...
Proof.
  induction n as [| n']. 
  - (* n = O *) intros m. rewrite -> plus_0_r. reflexivity.
...
Proof.
  ...
  induction m as [| m']. 
  - (* m = O *) simpl. rewrite -> plus_0_r. reflexivity.
     ...
     - Since there are three ways of giving evidence of gorgeousness
       ...
       principle generates three subgoals:
         ...
         - We must prove that, whenever [n] is a gorgeous
           ...
           then it also holds of [3+n] and [g_plus3 n e].

         - We must prove that, whenever [n] is a gorgeous
           number and [e] is an evidence of its gorgeousness,
           if [P] holds of [n] and [e],
           then it also holds of [5+n] and [g_plus5 n e].
...
(** The induction principle that Coq derives from this definition
    is the following: *)
...
(** This principle says that if [x] and [y] are equal then every
    property that is true of [x] is also true of [y], i.e. there is no
    context that distinguishes [x] and [y].  (Formally, this is the
    dual of a principle enunciated by Leibniz, which says that [x] and
    [y] are equal if they cannot be distinguished by any context. Note
    that the implication in this converse direction is trivially true
    in Coq: we simply instantiate [P] with the property of being equal
    to [x].  Leibniz took this principle itself as the _definition_ of
    equality, whereas in Coq it is a _consequence_ of defining
...
(** $Date: 2015-08-11 18:03:04 +0200 (Tue, 11 Aug 2015) $ *)
