...
Theorem beval_iff_bevalR : forall b bool,
  bevalR b bool <-> beval b = bool.
Proof.
  split.
  - intros; induction H; subst; try (constructor); simpl; 
    rewrite aeval_iff_aevalR' in H0; rewrite aeval_iff_aevalR' in H; rewrite H0; rewrite H; reflexivity.
  -  generalize dependent bool; generalize dependent b; induction b; try (intros; subst; constructor; apply aeval_iff_aevalR; reflexivity);
    intros; simpl beval in H; rewrite <- H; constructor.
...
Inductive no_whilesR: com -> Prop :=
 ...
 | No_Whiles_Ass  : forall v e, no_whilesR (v ::= e)
 | No_Whiles_Seq  : forall c1 c2, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (c1 ;; c2)
 | No_whiles_If   : forall c1 c2 e, no_whilesR c1 -> no_whilesR c2 -> no_whilesR (IFB e THEN c1 ELSE c2 FI)
.
...
Proof.
   split.
   - induction c; intros H; try constructor;
     try (apply IHc1; simpl in H; apply andb_true_iff in H; inversion H; apply H0);
     try (apply IHc2; simpl in H; apply andb_true_iff in H; inversion H; apply H1).
     inversion H.
...
Proof.
  intros. generalize dependent st. induction H; intros st.
    - exists st. constructor.
    - exists (update st v (aeval st e)). constructor. reflexivity.
    - destruct (IHno_whilesR1 st).
      destruct (IHno_whilesR2 x).
      exists x0.
      apply E_Seq with x. apply H1. 
      apply H2.
    - destruct (beval st e) eqn: If_conditional.
      + destruct (IHno_whilesR1 st). exists x. apply E_IfTrue. 
        * apply If_conditional.
        * apply H1.
      + destruct (IHno_whilesR2 st). exists x. apply E_IfFalse. 
        * apply If_conditional.
        * apply H1.
Qed.
(** [] *)
...
Fixpoint s_execute (st : state) (stack : list nat)
  ...
  match prog with 
  | []           => stack
  | cmd :: cmds  => match cmd with
    | SPush n           => s_execute st (n :: stack) cmds
    | SLoad v           => s_execute st (st v :: stack) cmds
    | SPlus             => match stack with
      | n :: m :: stack' => s_execute st (m + n :: stack') cmds
      | _                => s_execute st stack cmds
      end
...
Theorem cmd_seq_partition_dist_execute : forall stack st cmd_seq1 cmd_seq2,
  s_execute st stack (cmd_seq1 ++ cmd_seq2) = s_execute st (s_execute st stack cmd_seq1) cmd_seq2.
Proof.
 intros. 
 generalize dependent stack. generalize dependent cmd_seq2.  
 induction cmd_seq1.
 - reflexivity. 
 - intros. simpl. 
   Admitted.
...
Proof.
  intros.
  assert (forall stack, s_execute st stack (s_compile e) = aeval st e :: stack) as Generalization.
  {
    induction e; try reflexivity; 
    simpl; intros stack; rewrite cmd_seq_partition_dist_execute; 
    rewrite IHe1; rewrite cmd_seq_partition_dist_execute; 
...
Inductive ceval : com -> state -> status -> state -> Prop :=
  | E_Skip : forall st,
   ...
   | E_Break : forall (st : state),
      BREAK / st || SBreak / st
  | E_Ass : forall (st : state) (e : aexp) (n : nat) (x : id),
       aeval st e = n -> 
        (x ::= e) / st || SContinue / (update st x n)
  | E_Seq_Break : forall (c1 c2 : com) (st st' : state),
       (c1 / st || SBreak / st') -> 
         (c1 ;; c2) / st || SBreak / st'
  | E_Seq_Continue : forall (c1 c2 : com) (st st' st'': state) (s : status),
       (c1 / st || SContinue / st') -> 
         (c2 / st' || s / st'') -> 
           (c1 ;; c2) / st || s / st''
  | E_If_True : forall (st st': state) (s : status) (c1 c2 : com) (b : bexp),
       beval st b = true -> c1 / st || s / st' -> 
         (IFB b THEN c1 ELSE c2 FI) / st || s / st'
  | E_If__False : forall (st st': state) (s : status) (c1 c2 : com) (b : bexp),
       beval st b = false -> c2 / st || s / st' -> 
         (IFB b THEN c1 ELSE c2 FI) / st || s / st'
  | E_While_End : forall (st : state) (c1 : com) (b : bexp),
       beval st b = false -> 
         (WHILE b DO c1 END) / st || SContinue / st
  | E_While_Break : forall (st st' : state) (c1 : com) (b : bexp),
       beval st b = true -> 
         c1 / st || SBreak / st' ->
           (WHILE b DO c1 END) / st || SContinue / st'
  | E_While_Continue : forall (st st' st'': state) (s : status) (c1 : com) (b : bexp),
       beval st b = true -> 
         c1 / st || SContinue / st' ->
           (WHILE b DO c1 END) / st' || SContinue / st'' ->
             (WHILE b DO c1 END) / st || SContinue / st''
