...
Proof.
 ...
 inversion H1.
 - apply H2.
 - unfold not in H2. apply ex_falso_quodlibet. apply H2. unfold not in H0. 
...
Inductive all (X : Type) (P : X -> Prop) : list X -> Prop := 
  | empty_satisfies_all : all X P nil
  | non_empty_all : forall (x : X) (list : list X), P x -> all X P list -> all X P (x :: list)
.
...
Theorem forallb_true_implication_right : 
  forall (X : Type) (p : X -> bool) (l : list X), forallb p l = true -> all X (fun x => p x = true) l.
Proof.
  intros.
  induction l. 
  - intros. constructor.
  -  simpl. intros. constructor.
    + destruct (p x) eqn: Hpx.
      * reflexivity.
      * inversion H. rewrite Hpx. simpl. reflexivity.
    + apply andb_prop in H. inversion H. apply IHl in H1. apply H1.
Qed.
Theorem forallb_true_implication_left :
  forall (X : Type) (p : X -> bool) (l : list X), all X (fun x => p x = true) l -> forallb p l = true.
Proof.   
    intros.
    induction H.
    - reflexivity.
...
(* The above specification doesn't say anything about elements that doesn't satisfy predicate p
 *)
...
Inductive are_merge_of {X:Type} : list X -> list X -> list X -> Prop :=
 | merged_two_empty : are_merge_of [] [] []
 | merged_choose_first : forall x l1 l2 ml, are_merge_of l1 l2 ml -> are_merge_of (x::l1) l2 (x::ml)
 | merged_choose_second : forall x l1 l2 ml, are_merge_of l1 l2 ml -> are_merge_of l1 (x::l2) (x::ml).

Theorem filter_specification : 
forall (X : Type) (p : X -> bool) (l1 l2 l3 : list X),
  are_merge_of l1 l2 l3 ->
  all X (fun x => p x = true) l1 ->
  all X (fun x => p x = false) l2 ->
  filter p l3 = l1.
Proof.
  intros.
  induction H.
  - simpl. reflexivity.
  - simpl. apply forallb_true_implication_left in H0. simpl in H0. 
    apply andb_prop in H0. inversion H0.   
    apply forallb_true_implication_right in H3. rewrite H2.
    apply IHare_merge_of in H3.
    + rewrite H3. reflexivity.
    + apply H1.
  - inversion H1.  simpl. rewrite H4. apply IHare_merge_of.
    +  apply H0.
    + apply H5.
Qed.
