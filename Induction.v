...
Proof.
  ...
  - simpl. rewrite IHb. rewrite <- plus_n_O. rewrite <- plus_n_O.
    rewrite plus_assoc. 
    replace (bin_to_nat b + 1 + bin_to_nat b) with (bin_to_nat b + bin_to_nat b + 1).
    + reflexivity.
...
(* Answer to b:
  ...
  For example
   ...
   bin_to_nat zero = 0
   bin_to_nat (Twice zero) = 0

  Because of that we cannot prove 
...
(** $Date: 2015-08-10 19:55:52 +0200 (Mon, 10 Aug 2015) $ *)
