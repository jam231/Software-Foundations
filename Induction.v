...
Proof.
  ...
  Case "b = true".
    simpl. intro H_c_eq_true. apply H_c_eq_true.
  Case "b = false".
    simpl. intro H_c_eq_false.
...
Proof.
  intros n m p.
  induction p.
  simpl plus. intros H. rewrite H. reflexivity.
  intros H. simpl. rewrite IHp. reflexivity. 
...
Proof.
  ...
  induction n.
  simpl. reflexivity.
  intros m p. simpl. rewrite IHn.
  replace (m + (S (n + p))) with (S (n + p) + m).
  simpl. rewrite <- IHn. rewrite plus_assoc.
  replace (n + m + p) with (n + p + m). reflexivity.
  rewrite <- plus_comm.
  rewrite plus_assoc.
